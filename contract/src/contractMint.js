...
import { makeZoeHelpers, defaultAcceptanceMsg } from '@agoric/zoe/src/contractSupport/zoeHelpers';
...
export const makeContract = harden(zcf => {
  ...
  return zcf.addNewIssuer(issuer, 'Token').then(() => {
    ...
    const offerHook = offerHandle => {
      ...
      // For now the plastic type is a random value from types array
      const plasticType = types[Math.floor(Math.random() * types.length)];

      const payment = mint.mintPayment(plasticType);
